...
func apply(context: EnemyEffectContext) -> void:
	...
	var split := roundi(float(context.damage) * transfer_rate / float(targets.size())) # 譲渡値
	for enemy in targets: context.deal_acid_damage(enemy, split)
	context.damage = maxi(0, context.damage - split * targets.size())
