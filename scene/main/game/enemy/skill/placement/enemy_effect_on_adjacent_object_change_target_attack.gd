...
# 効果適用
func apply() -> void:
	for enemy in EnemyEffectTracking.get_activatable_new_adjacent(state, source, enemies, max_activations_per_target): enemy.add_damage(roundi(EnemyEffectValueCalculator.scale(source, float(attack_delta))))
