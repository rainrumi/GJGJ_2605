...
func apply_digest_damage(enemies: Array[Enemy], stomach: StomachBoard) -> int:
	var damage := 0
	for enemy in enemies:
		if enemy.can_take_stomach_turn():
...
func consume_pending_player_damage() -> int:
	var damage := _pending_player_damage
	_pending_player_damage = 0
	return damage
...
func _resolve_digested_enemy_effects(
	...
	for enemy in digested_enemies:
		...
		if _has_nightmare_effect(enemy, 10) and digest_order % 2 == 1:
			_pending_player_damage += seed_effects.apply_player_damage(enemy.get_damage() * 3, DIGEST_DAMAGE)
