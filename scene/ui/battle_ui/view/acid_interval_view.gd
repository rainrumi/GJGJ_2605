...
func _ready() -> void:
	...
	acid_interval_value_label.minimum_size_changed.connect(_update_minimum_width)
...
func _update_minimum_width() -> void:
	custom_minimum_size.x = (
		acid_interval_value_label.position.x
		+ acid_interval_value_label.get_combined_minimum_size().x
	)
