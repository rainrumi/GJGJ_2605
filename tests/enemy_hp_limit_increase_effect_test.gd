...
func _test_empty_cell_effect() -> void:
	...
	if max_hp_effect != null and hp_effect != null:
		_expect(max_hp_effect.max_hp_delta_per_cell == 100, "空きマスごとの最大HP増加量")
		_expect(hp_effect.hp_delta_per_cell == 100 and hp_effect.heal_over_maximum, "空きマスごとのHP増加量と上限外回復")
		...
		hp_effect.apply()
		enemy.data.hp.apply_modifiers()
		_expect(enemy.max_hp == original_max + 300, "空き3マスで最大HPが300増える")
		_expect(enemy.current_hp == original_max + 300, "空き3マスで現在HPが300増える")
		enemy.data.hp.reset_modifiers()
		max_hp_effect.apply()
		hp_effect.apply()
		enemy.data.hp.apply_modifiers()
		_expect(enemy.max_hp == original_max + 300, "再評価時の最大HP")
		_expect(enemy.current_hp == original_max + 300, "再評価でHPを重複加算しない")
		board.rows = 3
		enemy.data.hp.reset_modifiers()
		max_hp_effect.apply()
		hp_effect.apply()
		enemy.data.hp.apply_modifiers()
		_expect(enemy.max_hp == original_max + 500, "空き5マスに増えると最大HPも500増える")
		_expect(enemy.current_hp == original_max + 500, "空き5マスに増えると現在HPも500増える")
		board.rows = 2
		enemy.data.hp.reset_modifiers()
		max_hp_effect.apply()
		hp_effect.apply()
		enemy.data.hp.apply_modifiers()
		_expect(enemy.max_hp == original_max + 300, "空き3マスに戻ると最大HP補正も戻る")
		max_hp_effect.unbind()
		hp_effect.unbind()
	enemy.free()
	board.free()
