...
- [x] 所持枠の12枠を横4列・縦3行で表示する。
- [x] 装備枠の下端と所持枠の上端の間に20pxの縦余白を設ける。
...
- 装備上限は6、所持上限はなし。
- 所有種パネルの装備表示は3列2行、所持表示は4列3行・12個単位のページ式。
...
- [x] `scene/ui/seed/owned_seed_panel.*` に固定パネル、6装備枠、12所持枠、ページ矢印、閉じる操作を追加する。
