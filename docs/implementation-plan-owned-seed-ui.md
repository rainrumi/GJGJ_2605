...
- 装備6枠（3列×2行）と所持12枠（4列×3行）を30px角・間隔10pxで統一し、種あり枠は完全不透明の`#f0e0ff`、空き枠は透明背景と1pxの`#f0e0ff`縁、夢の種テクスチャは黒色で表示する。
- 所持枠はパネル左右から約20px離し、装備枠はパネル中央へ揃える。
- 装備枠の下端と所持枠の上端の間へ20pxの縦余白を設ける。
...
- [x] 装備リストを幅110pxで中央、所持リストを幅150px・左右21pxへ配置し、両リスト間を20px空けてページ操作が収まるよう調整する。
