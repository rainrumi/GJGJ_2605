...
- 装備6枠（3列×2行）と所持12枠（4列×3行）を30px角・間隔10pxで統一し、薄いピンクの半透明背景と黒い夢の種テクスチャを表示する。
...
- 枠サイズは「1.3倍程度」より「所持枠の左右余白約20px」を優先する。`(192 - 20×2 - 10×3) / 4 = 30.5`から30px角とする。
...
- [x] `seed_button.tscn` / `seed_button.gd`と`seed_button_list.*`を30px角・間隔10pxへ揃え、枠背景を薄いピンクalpha 0.3、外周線なしにする。
- [x] 装備リストを幅110pxで中央、所持リストを幅150px・左右21pxへ配置し、ページ操作が収まるよう調整する。
...
- [ ] `game.tscn`の通常表示をキャラクター中心X=95付近、頭頂部Y=54付近の3列×2行へ配置する。
- [ ] `battle_ui.gd`の通常時アイコン色を薄いピンクへ変更し、frame非表示を維持する。
- [ ] `dream_seed_inventory_test.gd`へ6個表示、30px透明当たり判定、薄いピンク色、ツールチップ、パネル開閉時の排他表示を追加する。
- [ ] 関連test、対象Scene smoke、開閉両状態のキャプチャを実行し、キャラクター・他の種・主要UIとの重なりを確認する。
...
6. `DREAM_SEED_CAPTURE_PATH`と`DREAM_SEED_CLOSED_CAPTURE_PATH`で640×360画像を保存し目視する。
